...
DEFAULT_MINIMUM_CLIENT_TIME = 120
...
def analyze_contract(
    job_index: int, index: int, contract_filename: str, result_queue, timeout
) -> None:
    """
    ...
    def calc_timeout(souffle_client=None):
        ...
        if (
            not args.disable_scalable_fallback
            and souffle_client == DEFAULT_DECOMPILER_DL
        ):
    ...
    def run_decomp(contract_filename, in_dir, out_dir, fallback_out_dir):
        ...
        if (
            not args.disable_precise_fallback
            and not def_timeouts
            and decomp_out_produced(out_dir)
        ):
            ...
            if (
                imprecise_decomp_out(out_dir)
                and calc_timeout(FALLBACK_PRECISE_DECOMPILER_DL) > 0.3 * timeout
            ):
                log(
                    f"Using precise fallback decompilation configuration for {os.path.split(contract_filename)[1]}"
                )

                pre_timeouts, _ = run_clients(
                    [FALLBACK_PRECISE_DECOMPILER_DL], [], in_dir, fallback_out_dir
                )
            ...
            if args.disable_scalable_fallback:
                ...
                sca_timeouts, _ = run_clients(
                    [FALLBACK_SCALABLE_DECOMPILER_DL], [], in_dir, out_dir
                )
    ...
    def decomp_out_produced(out_dir):
        ...
        ) and os.path.exists(join(out_dir, "Statement_Defines.csv"))

    try:
        # prepare working directory
        exists, work_dir, out_dir, fallback_out_dir = prepare_working_dir(
            contract_filename
        )
        ...
        if exists:
            ...
            shutil.copy(join(work_dir, "bytecode.hex"), join(out_dir, "bytecode.hex"))
            ...
            if os.path.exists(join(work_dir, "compiler_info.csv")):
                ...
                os.symlink(
                    join(work_dir, "compiler_info.csv"),
                    join(fallback_out_dir, "Verbatim_compiler_info.csv"),
                )

            timeouts, _ = run_clients(
                souffle_pre_clients, other_pre_clients, work_dir, work_dir
            )
            if timeouts:
            ...
            if args.context_depth is not None:
                write_context_depth_file(
                    os.path.join(work_dir, "InputMaxContextDepth.csv"),
                    args.context_depth,
                )
            ...
            decompiler_config = run_decomp(
                contract_filename, work_dir, out_dir, fallback_out_dir
            )
            ...
            if not args.disable_inline:
                ...
                run_clients(
                    [DEFAULT_INLINER_DL] * DEFAULT_INLINER_ROUNDS, [], out_dir, out_dir
                )
...
compile_processes_args.append(
    (DEFAULT_DECOMPILER_DL, DEFAULT_DECOMPILER_DL + SOUFFLE_COMPILED_SUFFIX)
)

if not args.disable_scalable_fallback:
    compile_processes_args.append(
        (
            FALLBACK_SCALABLE_DECOMPILER_DL,
            FALLBACK_SCALABLE_DECOMPILER_DL + SOUFFLE_COMPILED_SUFFIX,
        )
    )

if not args.disable_inline:
    compile_processes_args.append(
        (DEFAULT_INLINER_DL, DEFAULT_INLINER_DL + SOUFFLE_COMPILED_SUFFIX)
    )

if not args.disable_precise_fallback:
    compile_processes_args.append(
        (
            FALLBACK_PRECISE_DECOMPILER_DL,
            FALLBACK_PRECISE_DECOMPILER_DL + SOUFFLE_COMPILED_SUFFIX,
        )
    )
